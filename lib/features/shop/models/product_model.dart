...
class ProductModel {
  ...
  toJson() {
    return {
      ...
      "ProductVariation": productVariation != null
          ...
          : [],
      'SortId': sortId,
      'SearchKeywords': _generateKeywords(title, description, tags),
  ...
  factory ProductModel.fromSnapshot(
    ...
    return ProductModel(
      ...
      productType: _normalizeProductType(data["ProductType"]),
      productVisibility: ProductVisibility.values.firstWhere(
  ...
  List<String> _generateKeywords(
    ...
    String combined =
        "${title.toLowerCase()} ${desc?.toLowerCase() ?? ""} ${tags?.join(" ").toLowerCase() ?? ""}";
    // تقسيم النص إلى كلمات وتنظيفها من الرموز
    return combined
        .split(RegExp(r'\s+'))
        .where((word) => word.length > 1)
        .toSet()
...
class ProductModel {
  ...
  factory ProductModel.fromSnapshot(
    ...
    return ProductModel(
      ...
      price: double.parse((data["Price"] ?? 0.0).toString()),
      thumbnail: data["Thumbnail"] ?? "",
      brande: BrandModel.fromJson(data["Brande"]),
      categoryId: data["CategoryId"] ?? "",
